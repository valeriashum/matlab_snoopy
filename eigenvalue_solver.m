...
%                         'DisplayName','SNOOPY');
        ...
        hFig = figure(4);
        set(hFig, 'Position', [100, 60, 1200, 500]);
        set(gca, 'FontSize', 16)
        if (mod(n_max,2)~=0)
            %plot Rm vs max(rgr) and max(rgr)
            subplot(1,2,2)
            set(gca, 'FontSize', 16)
            loglog(rm_entry, rgr_max,...
                        'LineStyle', '-',...
                        'color',color{count_box},...
                        'LineWidth',1.5,...
                        'MarkerEdgeColor', color{count_box}, ...
...
%             legend(legendInfo,'Location','southeast','fontsize',16, 'Interpreter', 'latex','Box','off')
            %legend(legendInfo,'Location','southeast','fontsize',16, 'Interpreter', 'latex','Box','off')
            title({['ODD modes [-',num2str(n_max),',',num2str(n_max),']']},'interpreter','Latex', 'fontsize',18);
...
%                 'LineWidth',2.5);
            ...
            ylim([-0.01 0.05]);
            hold on; 
            legend(legendInfo,'Location','southeast','fontsize',14, 'Interpreter', 'latex');%,'Box','off')
            %legend(legendInfo,'Location','northeast','fontsize',20, 'Interpreter', 'latex','Box','off')
            %legend(legendInfo,'Location','outsidesouth')
            title({'$u_{m}$'},'interpreter','Latex', 'fontsize',20);  
...
p1 = loglog(ar_odd, rmc_odd, ... 
    ...
    'color','k',...
    'LineWidth',1.5,...
    'DisplayName','u_{m}^{odd} approximation');
xlim([1 110])
ylabel('R_m^{crit}');%,'fontsize',18);%, 'Interpreter', 'latex');
%set(gca, 'FontSize', 16)
xlabel('Aspect Ratio, L_x/L_z');
%ylabel('R_{m}^{crit}');
...
p2 = loglog(ar_even, rmc_even, ...
    ...
    'color','k',...
    'LineWidth',1.5,...
    'DisplayName','u_{m}^{even} approximation');
p3 = loglog(ar, sqrt(1./ar), ...
 ...
 set(gca, 'FontSize', 14)
p4 = loglog(arr_uii, rmc_uii*(1.5+m^2)/sqrt(1.5*(3+2*m^4+9*m^2)), 'o',...
    'LineStyle', 'none',...
    'color','k',...
    'LineWidth',1.5,...
    'MarkerEdgeColor', 'k', ...
...
p6 = loglog(arr_111, rmc_111, 'o',...
    ...
    'MarkerFaceColor', 'r', ...
    'MarkerSize',6);
%annotation('textarrow',[ 0.2 0.14],[0.67 0.67],'String','CUBE ')
set(gca, 'FontSize', 14)
...
%            hold on;
    subplot(1,2,2)
           ...
           hold on;
        hFig = figure(5);
        set(hFig, 'Position', [100, 60, 500, 400]);
        set(gca, 'FontSize', 16)
        plot(Rm, Gr,'o',...
            'LineStyle', ':',...
            'color',color{j},...
...
for j=1:size(files2,2)
    ...
    subplot(1,2,1)
           ...
           hold on;
    subplot(1,2,1)
            loglog(rm_entry, rm_entry./ar(j)-1./(ar(j)^2*rm_entry),'o',...
                        'LineStyle', '-',...
                        'color',color{j},...
                        'LineWidth',1.5,...
                        'MarkerEdgeColor', color{j}, ...
           ...
           hold on
            xlim([0 rm_entry(end)])
            ylim([-0.01 0.05]);
            title({'$u_{ABC}$'},'interpreter','Latex', 'fontsize',20); 
            xlabel('$R_m$', 'Interpreter', 'latex');
