...
for ct = 1:size(box_array,2)  % box number
    constttt=box_array(ct);
    for consttt=1:12   % RUN NUMBER
        rep= ['/store/ASTRO/vs391/kinematic_dynamo/u_abc/box_' num2str(constttt) '_kinematicOutput_' num2str(consttt)];
        rep1=['/store/ASTRO/vs391/kinematic_dynamo/u_iii/kinematicOutput_box_' num2str(constttt) '_' num2str(consttt)];
        for constt = 1:3 % file number
            for const = 0:7:7    %  number of cores
                fprintf('box_%d/run_%d/rates%d/rates_r0%d\n', constttt,consttt,constt, const);
                 clearvars -except box_array const constt consttt constttt folder rep rep1 ct
                 box_ar = constttt;
                 filename_out = [rep1, '/rates' num2str(constt) '/modes_r0' num2str(const) '.txt'];
              filename_out_mf = [rep1, '/rates' num2str(constt) '/modes_mf_r0' num2str(const) '.txt'];
                ...
                try
                   ...
                   clear tMF1 tMF2 tMF3 tMF4 tMF5 tMF6 tMF7;
                   count_mf=1;
                   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                   %%%%%%%%%%%%%%%%%%%%%%%%%%%  LOOP      %%%%%%%%%%%%%%%%%%%%%%%%%%%
                   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    for i=1:n_modes
                        ...
                        Bntfin{i} = tblB{mode_box{i+1}-1,6};
                        
                        % Save mean-field LS modes to a seperate document                 
                        if (abs(kx_arr{i}) < 1.0 && abs(ky_arr{i}) < 1.0 && abs(kz_arr{i}) < 1.0)
...
%                                     hold on;
%                                     legendInfo{i} = [num2str(kx_arr{i}) ',' num2str(ky_arr{i}) ',' num2str(kz_arr{i}) ];     
                               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%fprintf('Bn=%f\n',Bntfin{i});
                           ...
                           tMF1{count_mf} = box{i};
                           tMF2{count_mf} = rm{i};
                           tMF3{count_mf} = kx_arr{i};
                           tMF4{count_mf} = ky_arr{i};
                           tMF5{count_mf} = kz_arr{i};
                           tMF6{count_mf} = gr_arr{i}; 
                           tMF7{count_mf} = Bntfin{i}; 
                           count_mf=count_mf+1;
                        end
                    end

              
                tblMF_title={'Box','Rm','KX','KY','KZ','RGR', 'Bn'};
                tblMF= table(transpose(tMF1),...
                             transpose(tMF2),...
                             transpose(tMF3),...
                             transpose(tMF4),...
                             transpose(tMF5),...
                             transpose(tMF6),...
                             transpose(tMF7),...
                             'VariableNames', transpose(tblMF_title));

                writetable(tblMF, filename_out_mf,'Delimiter', 'tab');
                %legend(legendInfo)
                catch ME
